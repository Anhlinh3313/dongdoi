...
export const AwayLastMatches = ({ matchAnalysis, matche, teamData }) => {
  const router = useRouter();
  const [awayLastMatches, setAwayLastMatches] = useState([]);
  const [storageAwayLastMatches, setStorageAwayLastMatches] = useState([]);
  ...
  const handleChange = async (event) => {
    let data = storageAwayLastMatches;
    let listAwayLastMatches = data.slice(0, event);
    setAwayLastMatches(listAwayLastMatches);
    calculate(listAwayLastMatches);
  };
  ...
  const getAwayLastMatches = async () => {
    ...
    setAwayLastMatches(data);
    setStorageAwayLastMatches(data);
    calculate(data);
  };
  ...
  useEffect(() => {
    if (router.query?.pid) getAwayLastMatches();
  }, [router.asPath]);
  return (
    <Table
      columns={columns}
